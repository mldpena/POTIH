...
class InventoryWarning_Model extends CI_Model {
	...
	public function __construct() 
	{
		...
		$this->load->library('constants/warning_const');
		$this->load->library('sql');
		$this->load->helper('cookie');
		parent::__construct();
	}
	
	public function get_inventory_warning_list($param)
	{
		extract($param);
		...
		if ($branch != WARNING_CONST::ALL_OPTION) 
		{
		...
		if ($subgroup != WARNING_CONST::ALL_OPTION) 
		{
			$conditions .= " AND P.`subgroup_id` = ?";
			array_push($query_data,$subgroup);
		}

		if ($material != WARNING_CONST::ALL_OPTION) 
		{
		...
		if ($type != WARNING_CONST::ALL_OPTION) 
		{
			switch ($type) 
			{
				case 1:
					$type = WARNING_CONST::STOCK;
					break;
				
				case 2:
					$type = WARNING_CONST::NON_STOCK;
					break;
			}
		...
		switch ($orderby) 
		{
			case WARNING_CONST::ORDER_BY_NAME:
				$order_field = "P.`description`";
				break;
			
			case WARNING_CONST::ORDER_BY_CODE:
		...
		$query = "SELECT P.`id`, P.`material_code`, P.`description`,
						...
						END AS 'status',
						CASE 
							WHEN P.`type` = ".WARNING_CONST::NON_STOCK." THEN 'Non - Stock'
							WHEN P.`type` = ".WARNING_CONST::STOCK." THEN 'Stock'
						END AS 'type',
						...
						FROM product AS P
						LEFT JOIN material_type AS M ON M.`id` = P.`material_type_id` AND M.`is_show` = ".WARNING_CONST::ACTIVE."
						LEFT JOIN subgroup AS S ON S.`id` = P.`subgroup_id` AND S.`is_show` = ".WARNING_CONST::ACTIVE."
						LEFT JOIN product_branch_inventory AS PBI ON PBI.`product_id` = P.`id` $inventory_join
						WHERE (PBI.`inventory` > PBI.`max_inv` OR PBI.`inventory` < PBI.`min_inv`) AND P.`is_show` = ".WARNING_CONST::ACTIVE."
						$conditions
						$group_by
