...
class PurchaseOrder_Model extends CI_Model {
	...
	public function update_purchaseorder_head($param)
	{
		...
		$query_data 		= array($entry_date,$memo,$supplier_name,$orderfor,$this->_current_user,$this->_current_date,$this->_purchase_head_id);
	...
	public function search_purchaseorder_list($param)
	{
		...
		$query = "SELECT PH.`id`, COALESCE(B.`name`,'') AS 'location', COALESCE(B2.`name`,'') AS 'forbranch', 
					...
					LEFT JOIN branch AS B2 ON B2.`id` = PH.`for_branchid` AND B.`is_show` = ".PURCHASE_CONST::ACTIVE."
